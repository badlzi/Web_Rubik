function renderCartProducts() {
  ...
  for (let i = 0; i < dataLocal.length; i++) {
    ...
    if (
      dataLocal[i].name.toLowerCase().includes(inputSearch.toLowerCase()) ||
      dataLocal[i].price.toLowerCase().includes(inputSearch.toLowerCase())
    ) {
      ...
      const productRow = `
                <tr class="column-2" data-index=${i}>
                    ...
                    <td class="c2-r3">
                        <p class="cost">${formatCurrency(
                          parseFloat(dataLocal[i].price)
                        )}</p>
                    </td>
                    <td class="c2-r4" id=products-${i}>
                        <button style="text-align: center; width: 15%;" onclick="decrementValue(${i})" class="decrement">-</button>
                        <input style="text-align: center; width: 30%;" type="text" id="number-${i}" value="${
        dataLocal[i].quantity
                    ...
                    <td class="c2-r5">
                        <p class="money" id="subtotal-${i}">${formatCurrency(
        tamtinh
      )}</p>
                    </td>
                    <td class="c2-r6">
...
function updateTotalPrice() {
  ...
  for (let i = 0; i < dataLocal.length; i++) {
    ...
    if (cbo.checked == true) {
      ...
      selectCartProduct.push(dataLocal[i]);
    }
  ...
  localStorage.setItem("selectCartProduct", JSON.stringify(selectCartProduct));
...
function payment() {
  ...
  window.location.href =
    "http://localhost:3000/payment/payment.html?sum=" + totalSum;
}
